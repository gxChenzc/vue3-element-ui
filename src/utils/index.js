...
// export function getTableHeight(sh) {
//   if (sh) return window.innerHeight - 140 - sh
//   return window.innerHeight - 140
// }

export const getErrorRowObj = (data, props = []) => {
  try {
    let msg = data.msg || ''
    let result = data.data || ''
    if (result.indexOf('|') !== -1) {
      msg = result.substring(0, result.indexOf('|'))
      let str = result.substr(result.indexOf('|') + 1)
      let obj_arr = str.split('|')
      let json = JSON.parse(obj_arr[1] || '{}')
      let arr_index = []
      for (const key in json) {
        if (json.hasOwnProperty(key)) {
          if (props.indexOf(key) !== -1) {
            arr_index.push(props.indexOf(key))
          }
        }
      }
      return {
